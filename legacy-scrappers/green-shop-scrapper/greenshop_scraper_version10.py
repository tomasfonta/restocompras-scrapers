...
PRODUCT_ID = 1                               # <--- NUEVA CONSTANTE PARA EL PRODUCT ID HARCODEADO
...
def post_to_api(product_data):
    ...
    # 📢 Aquí se asegura que 'productId' sea 1, tomando el valor de la nueva constante
    payload = {
        "name": product_data["Nombre"],
        "description": product_data["Descripción"],
        "price": product_data["Precio_Num"], 
        "image": product_data["Imagen"],
        "productId": product_data["Producto ID"], # <--- Usa el valor inyectado en scrape_single_category
...
def scrape_single_category(url):
    ...
    for item in product_items:
        ...
        product_info = {
            ...
            "Imagen": image_url,
            "Producto ID": PRODUCT_ID, # <--- ASIGNACIÓN DEL VALOR HARCODEADO 1
            "Unidad": unit, 
            "Cantidad": quantity,
