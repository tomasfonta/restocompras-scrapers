...
OUTPUT_FILENAME = "greenshop_datos_finales.xlsx"
...
def parse_product_title(full_title):
    ...
    # Si la unidad es UNIT, establecer la cantidad a 1 (NUEVA REGLA)
    if unit == "UNIT":
        quantity = "1"
...
def scrape_single_category(url):
    ...
    for item in product_items:
        ...
        # Definiciones fijas o basadas en el Nombre
        marca = "Green Shop" # NUEVA REGLA
        description = name     # NUEVA REGLA
        product_id = "" 
...
def write_to_excel_file(data, filename):
    ...
    try:
        ...
        print(f"\n✅ Extracción y estandarización exitosa.")
        print(f"✅ Productos totales guardados: {len(data)}")
